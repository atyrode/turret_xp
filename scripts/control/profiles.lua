...
function restore_turret_item_state(entity, snapshot)
  ...
  for _, ammo in pairs(remaining) do
    if ammo.name and (ammo.count or 0) > 0 then
      ...
      if overflow > 0 then
        spill_stack_definition_at(entity.surface, entity.position, make_item_stack_definition(ammo.name, overflow, ammo.quality or "normal"))
      end
    end
  end
end
...
function make_bound_turret_item_stack(profile, turret_snapshot)
  ...
  return {
    name = get_bound_turret_item_name(profile),
    count = 1,
    quality = turret_snapshot.quality or "normal",
    tags = {
      [PROFILE_TAG] = serialized,
      [BOUND_TURRET_TAG] = copy_serializable(turret_snapshot)
    },
...
function read_bound_turret_stack(stack)
  if not stack or not stack.valid_for_read or not is_bound_turret_item_name(stack.name) then
    return nil, nil
  end
  ...
  pcall(function()
    profile_data = stack.get_tag(PROFILE_TAG)
  end)
  pcall(function()
    turret_data = stack.get_tag(BOUND_TURRET_TAG)
  end)

  local profile = deserialize_profile(profile_data)
  profile.bound_turret = true
  turret_data = type(turret_data) == "table" and copy_serializable(turret_data) or {}
  turret_data.quality = quality_name_from_stack(stack, turret_data.quality or "normal")
  turret_data.ammo = type(turret_data.ammo) == "table" and turret_data.ammo or {}
  return normalize_profile(profile), turret_data
end
...
function find_bound_turret_stack_in_inventory(inventory)
  ...
  for index = 1, #inventory do
    local stack = inventory[index]
    if stack and stack.valid_for_read and is_bound_turret_item_name(stack.name) then
      return stack
    end
  end
...
function get_bound_turret_stack_from_build_event(event)
  if event.stack and event.stack.valid_for_read and is_bound_turret_item_name(event.stack.name) then
    return event.stack
  end

  return find_bound_turret_stack_in_inventory(event.consumed_items)
end
...
function remove_bound_turret_mining_results(buffer, turret_snapshot)
  remove_item_from_inventory(buffer, {
...
function insert_bound_turret_item(inventory, entity, profile, turret_snapshot)
  local stack = make_bound_turret_item_stack(profile, turret_snapshot)
  local inserted = 0
  ...
  return spill_stack_definition(entity, stack)
end
