...
function update_core_panel(root, player, entity, state)
  ...
  if state.show_name_label == true then
    ...
    for index, channel in ipairs(channels) do
      local label = color_table.add({
        type = "label",
        caption = channel.label,
        style = "caption_label"
      })
      set_style(label, "font", "default-bold")
      set_style(label, "font_color", channel.color)
...
function add_active_custom_stats(stats, state, entity)
  ...
  if repair_rank > 0 then
    add_custom_stat(
      stats,
      "Regeneration",
      format_bonus_value_with_multiplier(get_repair_base_per_second(state, entity), get_specialization_multiplier(state, "repair_multiplier"), " HP/s", 1)
    )
  end
  ...
  if ammo_regen_rank > 0 then
    local caption = format_bonus_value_with_multiplier(ammo_regen_rank, get_specialization_multiplier(state, "ammo_recovery_multiplier"), " ammo/min", 0)
  ...
  if siphon_rank > 0 then
    add_custom_stat(
      stats,
      "Lifesteal",
      format_bonus_value_with_multiplier((siphon_rank * 0.004) * 100, get_specialization_multiplier(state, "lifesteal_multiplier"), " of damage", 1, "%")
    )
  end

  local bounce_rank = get_augment_rank(state, "bounce")
  if bounce_rank > 0 then
    add_custom_stat(stats, "Bullet bounce", rich_number(format_percent(apply_luck_to_chance(state, bounce_rank * 0.05), 1)) .. ", " .. rich_number("35%") .. " shot damage")
  end
...
function specialization_value_caption(specialization)
  ...
  local entries = ordered[specialization.id] or {
    { specialization.range_multiplier, " range" },
    { specialization.damage_multiplier, " damage" },
    { fire_rate_multiplier, " fire rate" },
    { specialization.health_multiplier, " HP" }
...
function sub_specialization_value_caption(sub_specialization)
  ...
  local entries = {
    ...
    { value = sub_specialization.cooldown_multiplier and (1 / sub_specialization.cooldown_multiplier) or nil, label = " fire rate", kind = "multiplier" },
...
function add_augment_allocation_row(parent, augment, rank, available, at_max)
  ...
  local rank_caption = augment.max_rank
    and ("Rank " .. tostring(rank) .. " / " .. tostring(augment.max_rank))
...
function add_element_mastery_panel(parent, state, element_id)
  ...
  local requirement = control_row.add({
    type = "label",
    caption = requirement and {
      "",
      "[item=", requirement.name, "] ",
      "Rank ",
      tostring(next_rank),
  ...
  set_style(requirement, "single_line", false)
  set_style(requirement, "maximal_width", LAYOUT.evolution_inner_width)
...
function add_first_element_section(parent, state)
  ...
  local section = add_section(
    ...
    GATES.first_element,
    nil,
    has_element and "Change" or nil,
    has_element and {
      turret_xp_action = "reset-element-slot",
      slot = 1
    } or nil,
    "Clear the first element so another one can be picked. This also clears the second element and their element ranks."
  )
  if unlocked then
...
function add_second_element_section(parent, state)
  ...
  local section = add_section(
    ...
    "Clear the second element so another one can be picked."
  )
  if not unlocked then
    return
  end
