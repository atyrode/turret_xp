...
function feeder.drop_position_matches_turret(inserter, turret)
  ...
  return math.abs((position.x or 0) - (turret_position.x or 0)) <= 1.4
    and math.abs((position.y or 0) - (turret_position.y or 0)) <= 1.4
end
...
function feeder.update_nearby_inserters(turret, state)
  ...
  for _, inserter in ipairs(inserters) do
    if feeder.inserter_points_at_turret(inserter, turret, feeder_entity) then
      ...
      local should_feed = needs_input and feeder_entity and feeder_entity.valid and (has_source_item or has_allowed_filter or managed ~= nil)
