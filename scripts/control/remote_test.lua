return function(M)
  ...
  turret_xp_test_register_methods(turret_xp_test_remote_methods, {
    ...
    last_extracted_core_reinstall_sample = function(entity)
      local inventory = game.create_inventory(8)
      local player = {
        index = -3,
        opened = entity,
        gui = {
      ...
      remember_open_turret(player, entity)
      install_profile_on_turret(
        entity,
        turret_xp_test_set_profile_fields(create_blank_profile(), {
          custom_name = "Undo",
          level = 9,
        })
      )
      local before = turret_xp_test_state_summary(entity)

      extract_core(player)
      local after_extract_state = turret_xp_test_state_summary(entity)
      ...
      local after_reinstall = turret_xp_test_state_summary(entity)
      local record_after_reinstall = get_last_extracted_core(player, entity)

      extract_core(player)
      local second_record = get_last_extracted_core(player, entity)
      for index = 1, #inventory do
        inventory[index].clear()
      end
      reinstall_last_extracted_core(player)
      local after_missing_reinstall = turret_xp_test_state_summary(entity)
      local record_after_missing = get_last_extracted_core(player, entity)
      ...
      return {
        before = before,
        after_extract = {
          installed = after_extract_state,
          record_chip_id = record and record.chip_id or nil,
          option_slot = option and option.index or nil,
          option_name = option and option.profile and option.profile.custom_name or nil,
        },
        after_reinstall = after_reinstall,
        record_after_reinstall = record_after_reinstall,
        second_record_chip_id = second_record and second_record.chip_id or nil,
        after_missing_reinstall = after_missing_reinstall,
        record_after_missing = record_after_missing,
      }
    end,
