...
function start_element_rank_project(entity, state, element_id, slot)
  ...
  local evolution = ensure_evolution_state(state)
  slot = slot or 1
  if evolution.elements[1] == element_id then
    slot = 1
  elseif evolution.elements[2] == element_id then
    slot = 2
  end
...
function allocate_augment(player, augment_id, amount)
  ...
  sync_turret_progression(state)
  if augment_id == "range" or augment_id == "max_health" then
    combat.mark_turret_body_sync_pending(state)
  end
  refresh_open_turret(player, entity, anchor)
end
...
function reset_augments(player)
  ...
  refresh_open_turret(player, entity, evolution_anchor_name("augment", "range"))
end
...
function reset_element_slot(player, slot)
  ...
  refresh_open_turret(player, entity, evolution_anchor_name("element", slot == 1 and "explosive" or (evolution.elements[1] or "explosive"), slot))
end
