...
local PASS_TICK = 900
local TEST_PREFIX = "[turret_xp_headless_tests] "

local function fail(message)
  error(TEST_PREFIX .. message, 2)
end
...
local function assert_eq(actual, expected, message)
  if actual ~= expected then
    fail(message .. " (expected " .. serpent.line(expected) .. ", got " .. serpent.line(actual) .. ")")
  end
end

local function assert_gt(actual, minimum, message)
...
local function list_count(list, value)
  local count = 0
...
local function assert_contains(list, value, message)
  if list_count(list, value) <= 0 then
    fail(message .. " (missing " .. serpent.line(value) .. " in " .. serpent.line(list or {}) .. ")")
  end
end
...
local function clear_test_area(surface)
  for _, entity in pairs(surface.find_entities_filtered({ area = area_around({ x = 0, y = 0 }, 96) })) do
...
local function ground_item_count(surface, position, item_name, radius)
  local count = 0
  for _, entity in pairs(surface.find_entities_filtered({
    area = area_around(position, radius or 1.5),
    type = "item-entity"
  })) do
...
local function find_ground_stack(surface, position, item_name, radius)
  for _, entity in pairs(surface.find_entities_filtered({
    area = area_around(position, radius or 1.5),
    type = "item-entity"
  })) do
...
local function find_stack(inventory, item_name)
...
local function create_turret(surface, position, ammo_count)
  ...
  assert_true(turret and turret.valid, "failed to create test gun turret")
...
local function find_turret_near(surface, position)
...
local function require_turret_near(surface, position, message)
  local turret = find_turret_near(surface, position)
  assert_true(turret ~= nil, message)
  return turret
end

local function call(method, ...)
  assert_true(remote.interfaces[IFACE] ~= nil, "missing Turret XP headless test remote interface")
  assert_true(remote.interfaces[IFACE][method] ~= nil, "missing Turret XP headless test method " .. method)
  return remote.call(IFACE, method, ...)
end
...
local function run_evolution_body_test(surface)
  ...
  assert_eq(summary.evolution.augments.range, 3, "range augment rank did not persist")
  assert_eq(summary.entity_name, "turret-xp-gun-turret-sniper-range-3", "specialization/range did not swap to the expected turret body")
...
local function run_max_health_variant_test(surface)
  local turret_position = { x = 42, y = 0 }
  local turret = create_turret(surface, turret_position, 10)
  local summary = call("install_core", turret, { level = 25 })
  assert_true(summary ~= nil, "failed to install core for max health variant test")
  ...
  assert_eq(summary.evolution.augments.max_health, 4, "max health rank did not persist in augment state")
  assert_near(summary.derived.repair_per_second, 0.6, 0.0001, "regeneration did not scale from current max health")
end
...
local function run_targeted_reset_test(surface)
  ...
  assert_eq(summary.evolution.augments.range, 2, "base reset incorrectly changed augments")
  ...
  assert_eq(summary.evolution.augments.range or 0, 0, "augment reset did not clear range ranks")
...
local function run_full_evolution_reset_test(surface)
  ...
  assert_eq(summary.evolution.augments.range or 0, 0, "full reset did not clear augment ranks")
...
local function run_bound_turret_test(surface)
  ...
  assert_eq(preview_decoded.profile.evolution.augments.range, 3, "specialized spilled bound turret lost its range rank")
...
script.on_init(function()
  ...
  run_immediate_tests()
  log(TEST_PREFIX .. "immediate assertions passed")
end)
