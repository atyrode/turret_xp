...
- Keep the installed-core header progressive: live mode should breathe and show only identity, label/name controls, the XP panel, one unframed Build mode entry row, and Follow build only while a saved target is unfinished. Target level, core and augment budgets, specialization, element, and formula details belong inside Build mode only; loop priorities belong on their own labeled Evolution row under each upgrade or augment, not as duplicate summary rows.
