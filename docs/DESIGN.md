...
- Keep the installed-core header progressive: live mode should breathe and show only identity, label/name controls, the XP panel, the Build mode entry row, and Follow build when a target exists. Target level, core and augment budgets, specialization, element, and formula details belong inside Build mode only; loop priorities belong on their Evolution rows, not as duplicate summary rows.
