...
- Keep the Stats pane visually parallel to Evolution: a fixed header and a bounded scroll body with compact section headers for identity, defense, offense, ammo, history, and active effects.
...
- Keep platform core selection explicit: when multiple tagged cores are in a platform hub, the player chooses the exact row using level, specialization, and neutral preview stats rather than lifetime history counters or buff/penalty colors.
