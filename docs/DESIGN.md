...
- When no core is installed, use the whole Turret XP body as an exact core picker: keep the scripted Veteran Core slot and short explanation at the top, then dedicate the remaining space to tagged Veteran Cores from the player's inventory. The list should read as a Factorio table: clickable headers for name, level, HP, attack speed, and range cycle active direction with a compact table-header sprite cue and then clear back to the default strongest-core ordering; `All` is the default filter and specific Base/Sniper/Machine gun/Bulwark/Brawler checkboxes narrow visible rows; unnamed cores stay last when sorting by name. Rows should use rich-value captions for level and stat previews, shared specialization colors, native table row styling, right-aligned stat columns, and a compact `+` action for the exact inventory slot. Kills and lifetime damage are historical context, not picker-decision columns.
