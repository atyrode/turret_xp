...
- When no core is installed, use the whole Turret XP body as an exact core picker: keep the scripted Veteran Core slot and short explanation at the top, then dedicate the remaining space to tagged Veteran Cores from the player's inventory. The list should read as a Factorio table: clickable headers for name, level, kills, and damage cycle active direction with a sprite cue and then clear back to the default strongest-core ordering; Base/Sniper/Machine gun/Bulwark/Brawler checkboxes filter visible rows; unnamed cores stay last when sorting by name. Rows should use rich-value captions for level/history/stat values, shared specialization colors, visible delimiters, right-aligned stat columns, and a compact `+` action for the exact inventory slot.
...
- Keep floating-label controls top-down and conditional: name field plus `Show`, a small square color swatch next to the preset/custom trigger, RGB sliders only when the label is shown, then the `Level` option. The native train/player color picker is not exposed to runtime mod GUIs, so Turret XP should mimic the useful visual language without implying the engine popup is available.
