...
- When no core is installed, use the whole Turret XP body as an exact core picker: keep the scripted Veteran Core slot and short explanation at the top, then dedicate the remaining space to tagged Veteran Cores from the player's inventory. The list should read as a Factorio table: clickable headers for name, specialization, level, HP, attack speed, and range cycle active direction with a compact right-side table-header sprite cue and then clear back to the default strongest-core ordering; `All` is the default filter and specific Base/Sniper/Machine gun/Bulwark/Brawler checkboxes narrow visible rows; unnamed cores stay last when sorting by name. Rows should use rich-value captions for level and stat previews, relative red/orange/yellow/green coloring against the currently visible core rows, a separate specialization column with shared specialization colors, native table row styling, right-aligned stat columns, and a compact `+` action for the exact inventory slot. Kills and lifetime damage are historical context, not picker-decision columns.
- Keep platform core selection explicit: when multiple tagged cores are in a platform hub, the player chooses the exact row using level, specialization, and preview stats rather than lifetime history counters.
...
- Keep floating-label controls top-down and conditional: name field plus `Show`, then a small square color swatch next to a color-picker trigger. The trigger opens Turret XP's own draggable `player.gui.screen` popup for presets, RGB sliders, and the `Level` option. The native train/player color picker is not exposed to runtime mod GUIs, so Turret XP should mimic the swatch-plus-picker interaction without implying the engine popup is available.
...
- Prefer `gui_support` rich-value and specialization-caption helpers for repeated numeric and identity captions so level, history, stat previews, formulas, summaries, and specialization labels do not hand-roll rich-text color tags.
