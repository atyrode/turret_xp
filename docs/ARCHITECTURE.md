...
- `prototypes/`: data-stage modules for names, items, bound-turret placeholder and preview variants, feeder, hidden core requester, styles, effects, and turret variants.
...
storage.turret_xp = {
  turrets = {
    [unit_number] = {
      chip_id = <string>,
      entity = <LuaEntity>,
      request_core = <boolean>,
      pending_policy = <serialized_setup_policy>,
      core_requester = <LuaEntity>,
      core_requester_unit_number = <uint>,
  ...
  core_requesters = {
    [unit_number] = <turret_unit_number>
  },
  core_requester_refresh = {
    tick = <uint>,
    active = <uint>,
    processed = <uint>,
    installed = <uint>
  ...
  player_settings = {
    [player_index] = {
      ...
      core_picker_sort = "none"|"level:asc"|"level:desc"|"name:asc"|"name:desc"|"specialization:asc"|"specialization:desc"|"hp:asc"|"hp:desc"|"attack:asc"|"attack:desc"|"range:asc"|"range:desc",
      core_picker_filters = {
        all = <boolean>,
        base = <boolean>,
        sniper = <boolean>,
...
- Empty copied-build request ownership: `scripts/control/core_requester.lua` owns hidden logistic requester lifecycle, the one-core logistic request section, bounded refresh processing, delivered-core installation, request diagnostics, and teardown spilling. `prototypes/core_requester.lua` defines the hidden requester helper. Request helpers are valid only for empty gun turrets whose host record has pending copied setup policy and `request_core = true`; installing, mining, or deleting the turret must unregister and destroy the helper.
- Player inventory core transfer: when no core is installed, the Turret XP panel switches to a single full-width Veteran Core picker. It lists tagged Veteran Cores from the player's main inventory, default-sorted by level descending with persistent per-player tri-state sort headers for level, name, specialization, HP, attack speed, and range, plus a persistent `All` filter and per-player base/specialization filters. HP, attack, and range sorting is resolved in the GUI layer from the opened turret's current quality/ammo/stat formulas; the profile-inventory helper remains responsible for tag-preserving stack discovery and exact per-slot `+` install actions and exposes a one-scan picker model so refresh keys and rendering do not rediscover the same stacks. The selected stack is removed only after its profile has been read, so tags stay authoritative. While the empty picker is open, supported runtimes use `on_player_main_inventory_changed` to refresh the picker immediately after inventory changes; the normal keyed GUI refresh remains the fallback.
...
- Profile ownership: `scripts/control/profile_schema.lua` owns Veteran Core defaults, normalization, copying, and current serialized schema shape. `scripts/control/profile_tags.lua` owns Veteran Core item tag read/write and tooltip build lines. `scripts/control/profile_inventory.lua` owns core stack insertion/spill, sorted player-inventory and platform-hub core lookup, bound turret health/ammo snapshots, ammo reconciliation, and turret item-state restoration. `scripts/control/profile_labels.lua` owns optional name/level/unspent labels and the Shield render bar lifecycle. `scripts/control/profile_service.lua` owns installed-core allocation, live turret host lookup, install, detach, state removal, and request-helper cleanup during core installation/removal. `scripts/control/profile_automation.lua` owns Build mode target normalization, deterministic finite/forever target spending, copied setup policies, target summary models, and pending policy application after a requested core is delivered. `profiles.lua` wires these explicit services into the legacy shared runtime API while other modules migrate off `_ENV` names.
...
- Combat XP pacing: raw `damage` and `kill_credit` remain display totals, but XP counters are weighted by combat context. Non-asteroid combat on space-platform surfaces applies a surface reduction. Asteroids and asteroid chunks use separate travelling and stopped platform target reductions that do not stack with that surface reduction. Veteran Training is applied when new combat XP is earned, while larger enemies, worms, and spawners can pay more kill-credit XP than small enemies.
...
- GUI support ownership: `scripts/control/gui/shell.lua` owns the `flib.gui`-built top-level Turret XP frame, native frame caption, relative turret-GUI anchor, snapshot-only screen root, fallback root, and empty/installed column modes. Relative GUI anchors cannot be dragged as screen frames, so draggable popups such as the label color picker live in `player.gui.screen`; the attached panel keeps the native frame caption so it follows vanilla turret-window movement. `scripts/control/gui/actions.lua` owns GUI widget action dispatch for click, checked-state, value-changed, text-changed, and selection-state events, including exact inventory/platform Veteran Core install actions, picker sort/filter changes, label color popup actions, and Build mode controls. `scripts/control/gui/runtime.lua` owns opened-turret GUI context assembly, Build mode preview projection, and mode-aware full/stat-only refresh orchestration. `scripts/control/gui/widgets.lua` owns compact local widget builders such as icon-led tool buttons. `scripts/control/gui/core_identity.lua` owns the Veteran Core slot/status header, installed extract/bind actions, pending copied-build core fulfillment slot, and the empty-core dev-create shortcut. `scripts/control/gui/core_label_controls.lua` owns installed-core naming, independent floating-label component toggles, swatch, and color trigger. `scripts/control/gui/core_automation_controls.lua` owns installed-core Build mode, live Follow build, and copied-target summary controls. `scripts/control/gui/core_platform_controls.lua` owns space-platform hub core list rows and installed-core send-to-hub controls, using neutral preview stats so platform choices follow the same rich-text semantics as the inventory picker. `scripts/control/gui_support.lua` owns generic GUI rich-text formatting, rich-value metric helpers, shared specialization color captions, and repeated Evolution width helpers. `scripts/control/gui_components.lua` owns reusable panel primitives such as fixed-header content panes, left-column section frames and their Build mode variants, subheader frames, shallow nested section frames, stat section headers, stat rows, stat tables, Evolution section shells, rank steppers, rank-allocation row budgets, summary labels, delimiters, and generic choice rows. `scripts/control/gui/core_picker_table.lua` owns the full-width Veteran Core picker table contract: headers, row cell composition, column widths, sort arrow placement, native table line settings, action-cell sizing, and action-column cue. `scripts/control/gui_panels.lua` is now a compatibility coordinator that exposes only the runtime-facing GUI panel/update entrypoints plus legacy helpers still consumed by non-GUI services: `core_panel.lua` coordinates XP/core/player-inventory picker/platform/dev sections, pending copied-build fulfillment, installed-core label controls, and Build mode controls, reuses picker inventory scans between refresh keys and rendering, `stats_panel.lua` owns grouped stat content and ammo/stat rendering inside the left-column fixed-header pane, `evolution_panel.lua` owns the right-column Evolution pane, shared rank-allocation rows, specialization effect cards, element choice rows, selected choice detail cards, and keyed refresh flow inside the shared fixed-header pane, and `formatters.lua` owns shared GUI-facing formatter captions such as specialization effects, element summaries, and combo captions.
...
- `on_player_setup_blueprint`: writes Turret XP setup policy tags for gun turrets so blueprinted/copy-built empty turrets can request or manually receive a core and inherit label/automation setup, copied build targets, and copied bound state without cloning the source core profile.
- `on_built_entity`, `on_robot_built_entity`, and `on_space_platform_built_entity`, when available: restore bound veteran turret profiles from tagged placeable turret items after converting the bound-only placeholder into a real gun turret, and apply Turret XP setup policy tags to normal gun turrets.
...
- `on_entity_settings_pasted`: copies Turret XP setup policy and target-build data between gun turrets without copying XP/history/name-bearing profile state.
- `on_gui_opened`: attach the Turret XP panel to the opened vanilla gun turret GUI. Pristine empty turrets build a single full-width Veteran Core picker mode with the scripted core slot and inventory picker. Empty turrets with a pending copied build show a pending core slot and fulfillment summary instead of the picker. Installed-core turrets build a fixed-width two-column mode: core, automation, XP, dev controls, and a shallow Stats content pane with a static header and bounded scroll body on the left; a shallow Evolution content pane with a static summary header and bounded scroll body on the right.
- `on_gui_closed`: remove the panel.
- GUI widget routing is table-driven in `scripts/control/gui/actions.lua`. Mutations that follow the common opened-turret lookup, state mutation, and GUI refresh flow should use the explicit `scripts/control/actions.lua` service and its `opened_turret_action` helper instead of repeating that transaction shape.
...
- The anchored top-level GUI frame and empty/installed shell modes belong in `scripts/control/gui/shell.lua`; GUI widget action dispatch belongs in `scripts/control/gui/actions.lua`; opened-turret GUI context and refresh orchestration belong in `scripts/control/gui/runtime.lua`; compact local widget builders belong in `scripts/control/gui/widgets.lua`; generic GUI formatting, rich values, and repeated width helpers belong in `scripts/control/gui_support.lua`; reusable GUI primitives belong in `scripts/control/gui_components.lua`; domain-specific panel sections belong in focused services under `scripts/control/gui/`, with `gui_panels.lua` kept as the compatibility wiring layer while legacy event/action code still calls shared names.
