...
storage.turret_xp = {
  ...
  players = {
    [player_index] = {
      entity = <LuaEntity>,
      unit_number = <uint>,
      core_picker_sort = "none"|"<field>:asc"|"<field>:desc",
      core_picker_filters = {
        base = <boolean>,
        sniper = <boolean>,
        machine_gun = <boolean>,
...
- Player inventory core transfer: when no core is installed, the Turret XP panel switches to a single full-width Veteran Core picker. It lists tagged Veteran Cores from the player's main inventory, default-sorted by level descending with per-player tri-state sort headers for level, kills, damage, and name, plus per-player base/specialization filters and exact per-slot `+` install actions. The selected stack is removed only after its profile has been read, so tags stay authoritative.
