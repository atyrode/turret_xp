...
- The anchored top-level GUI frame belongs in `scripts/control/gui/shell.lua`; GUI widget action dispatch belongs in `scripts/control/gui/actions.lua`; generic GUI formatting and repeated width helpers belong in `scripts/control/gui_support.lua`; reusable GUI primitives belong in `scripts/control/gui_components.lua`; domain-specific panel sections belong in focused services under `scripts/control/gui/`, with `gui_panels.lua` kept as the compatibility wiring layer while legacy event/action code still calls shared names.
