...
storage.turret_xp = {
  ...
  players = {
    [player_index] = {
      entity = <LuaEntity>,
      unit_number = <uint>,
      core_picker_sort = "none"|"level:asc"|"level:desc"|"name:asc"|"name:desc"|"hp:asc"|"hp:desc"|"attack:asc"|"attack:desc"|"range:asc"|"range:desc",
      core_picker_filters = {
        all = <boolean>,
        base = <boolean>,
        sniper = <boolean>,
...
- Player inventory core transfer: when no core is installed, the Turret XP panel switches to a single full-width Veteran Core picker. It lists tagged Veteran Cores from the player's main inventory, default-sorted by level descending with per-player tri-state sort headers for name, level, HP, attack speed, and range, plus an `All` filter and per-player base/specialization filters. HP, attack, and range sorting is resolved in the GUI layer from the opened turret's current quality/ammo/stat formulas; the profile-inventory helper remains responsible for tag-preserving stack discovery and exact per-slot `+` install actions. The selected stack is removed only after its profile has been read, so tags stay authoritative.
