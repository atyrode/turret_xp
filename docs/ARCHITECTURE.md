...
- `scripts/control/`: runtime modules for storage, profiles, progression, feeder logistics, stats, GUI, core-slot actions, combat effects, events, commands, headless-test remotes, and explicit helper/service modules such as Factorio API compatibility, label-color matching, bound turret item handling, migration compatibility, damage accounting, combat effect budgets, feeder routing, stat formatting/math, GUI action handlers, command registration, progression definitions, runtime constants, GUI constants, generic GUI support, and reusable GUI components.
