...
- Player inventory core transfer: when no core is installed, the left-column Veteran Core panel lists tagged Veteran Cores from the player's main inventory, sorted by level descending, with exact per-slot `+` install actions. The selected stack is removed only after its profile has been read, so tags stay authoritative.
...
- The anchored top-level GUI frame belongs in `scripts/control/gui/shell.lua`; GUI widget action dispatch belongs in `scripts/control/gui/actions.lua`; opened-turret GUI context and refresh orchestration belong in `scripts/control/gui/runtime.lua`; compact local widget builders belong in `scripts/control/gui/widgets.lua`; generic GUI formatting and repeated width helpers belong in `scripts/control/gui_support.lua`; reusable GUI primitives belong in `scripts/control/gui_components.lua`; domain-specific panel sections belong in focused services under `scripts/control/gui/`, with `gui_panels.lua` kept as the compatibility wiring layer while legacy event/action code still calls shared names.
