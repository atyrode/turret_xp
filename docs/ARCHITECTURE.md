...
- Empty-core request ownership: `scripts/control/core_requester.lua` owns hidden logistic requester lifecycle, the one-core logistic request section, bounded refresh processing, delivered-core installation, request diagnostics, and teardown spilling. `prototypes/core_requester.lua` defines the hidden requester helper. Request helpers are valid only for empty gun turrets whose host record has `request_core = true`; installing, cancelling, mining, or deleting the turret must unregister and destroy the helper.
...
- `on_player_setup_blueprint`: writes Turret XP setup policy tags for gun turrets so blueprinted/copy-built empty turrets can request a core and inherit label/automation setup plus copied build targets without cloning the source core profile.
