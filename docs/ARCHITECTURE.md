...
- `on_gui_opened`: attach the Turret XP panel to the opened vanilla gun turret GUI. Empty turrets build a single full-width Veteran Core picker mode with the scripted core slot and inventory picker. Installed-core turrets build a fixed-width two-column mode: core, XP, dev controls, and a shallow Stats content pane with a static header and bounded scroll body on the left; a shallow Evolution content pane with a static summary header and bounded scroll body on the right.
