...
- `scripts/control/`: runtime modules for storage, profiles, progression, feeder logistics, stats, GUI, core-slot actions, combat effects, events, commands, headless-test remotes, and explicit helper modules such as label-color matching, bound turret item handling, damage accounting, and generic GUI support.
...
- GUI support ownership: `scripts/control/gui_support.lua` owns generic GUI rich-text formatting and repeated Evolution width helpers. `gui_panels.lua` still owns domain-specific panel composition, wording, action tags, and refresh behavior.
