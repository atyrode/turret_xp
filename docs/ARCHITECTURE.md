...
storage.turret_xp = {
  ...
  players = {
    [player_index] = {
      entity = <LuaEntity>,
      unit_number = <uint>,
      core_picker_sort = "level"|"kills"|"damage"|"name"
    }
  },
  player_settings = {
    [player_index] = {
...
- Player inventory core transfer: when no core is installed, the Turret XP panel switches to a single full-width Veteran Core picker. It lists tagged Veteran Cores from the player's main inventory, default-sorted by level descending with per-player sort tabs for level, kills, damage, and name, with exact per-slot `+` install actions. The selected stack is removed only after its profile has been read, so tags stay authoritative.
...
- GUI support ownership: `scripts/control/gui/shell.lua` owns the `flib.gui`-built top-level Turret XP frame, relative turret-GUI anchor, fallback root, header drag target, and empty/installed column modes. `scripts/control/gui/actions.lua` owns GUI widget action dispatch for click, checked-state, value-changed, and text-changed events, including exact inventory/platform Veteran Core install actions and picker sort changes. `scripts/control/gui/runtime.lua` owns opened-turret GUI context assembly and mode-aware full/stat-only refresh orchestration. `scripts/control/gui/widgets.lua` owns compact local widget builders such as icon-led tool buttons. `scripts/control/gui_support.lua` owns generic GUI rich-text formatting, rich-value metric helpers, and repeated Evolution width helpers. `scripts/control/gui_components.lua` owns reusable panel primitives such as stat rows, stat tables, Evolution section shells, summary labels, delimiters, and generic choice rows. `scripts/control/gui_panels.lua` is now a compatibility coordinator that wires focused services under `scripts/control/gui/`: `core_panel.lua` owns XP/core/player-inventory core picker/platform/dev controls, `stats_panel.lua` owns the fixed-header stat pane and ammo/stat rendering, `evolution_panel.lua` owns Evolution sections and keyed refresh flow, and `formatters.lua` owns shared GUI-facing formatter captions such as specialization effects, element summaries, and combo captions.
