...
0.4.4 is published on the Factorio Mod Portal.
...
15. Hover allocation buttons and confirm the tooltip describes the specific upgrade and next rank.
16. Use `Respec` and confirm core upgrade and augment point ranks reset while element/specialization choices stay.
17. Use dev `Reset` and confirm the installed core returns to a fresh zero-XP state.
18. At level 10, start an Explosive, Fire, or Electric element project.
19. Confirm the active project shows item requirements and a progress bar.
20. Confirm a Veteran Core feeder appears near the turret and the Evolution panel shows its feeder status.
21. Insert the required material into the feeder manually or with an inserter and confirm the project progress advances without a deposit button.
22. Use `Materials` and confirm the element becomes active, or advances the selected element's next mastery milestone if no unlock project is active.
23. At level 20, pick Sniper, Machine Gun, Bulwark, or Brawler and confirm the choice is locked in and the turret stats change.
24. At level 30, buy a powerful augment and confirm augment points are earned every ten levels.
25. At level 40, start and complete a second element project and confirm the combo text appears.
26. Extract or mine the core and confirm leftover feeder contents spill instead of disappearing.
27. Let the turret shoot enemies and confirm XP, damage dealt, kills, and upgrade effect feedback feel visible enough to judge.
28. Damage a turret with regeneration ranks and confirm passive repair works.
29. Try Lifesteal in combat and check whether vampiric healing is understandable.
30. Change the Turret XP runtime-global settings and confirm the open panel refreshes with the new XP pacing.
31. Select the turret and run `/turret-xp` as a fallback path.
