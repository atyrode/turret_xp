...
- A local manual sandbox companion now exists for development playtesting: `scripts/sandbox.sh install` installs the current package plus `turret_xp_sandbox`, and `/turret-xp-sandbox build` creates a disposable scenario surface for core movement, Evolution, feeder routing, combat/stat behavior, and automation setup policy.
...
- Manual sandbox review: `scripts/sandbox.sh install`, `/turret-xp-sandbox build` in a disposable graphical development save, then use `/turret-xp-sandbox list` and `/turret-xp-sandbox goto <id>` to inspect focused manual scenarios.
