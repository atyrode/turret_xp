...
- Installed-core GUI layout now uses a two-row shell: core identity/XP/dev controls sit on the top-left, Stats sits on the top-right, and Evolution gets a full-width lower pane with Core, Specialization, Elements, and Augments tabs.
- Evolution base upgrades now render as three full-width Core-tab columns: Offense, Defense, and Support. Augment rows still share the full-width rank-allocation row builder with explicit icon, detail, value, and stepper widths derived from the Evolution viewport.
