...
- Runtime code has been split into focused modules under `scripts/control/`, with explicit helper/service modules for Veteran Core profile schema/tags/inventory/labels/orchestration, bound turret item handling, damage accounting, combat effect budgets, GUI support/components, Factorio API compatibility, label color matching, feeder routing, stat formatting/math, GUI actions, and command registration.
