...
- Installed-core Build mode now supports planning core ranks, augment ranks, specialization, sub-specialization, elements, and forever priorities without spending live points. Auto follows that build path, stays enabled for open-ended forever priorities, and unticks once finite paths are satisfied. Headless coverage protects build editing, Auto read-only behavior, copied target policies, requester delivery, and GUI dispatch.
...
- Blueprint/setup policy copy now carries label visibility, label color, build targets, Build-mode Auto state, and empty-core request settings without copying XP/history/name-bearing profile data.
