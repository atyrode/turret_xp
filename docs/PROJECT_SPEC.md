...
- Shield adds 50 shield per rank, absorbs incoming damage before turret HP, starts recharging after a short delay without damage, and shows a two-row in-world status stack while relevant: HP above, Shield below, equal width, neutral dark frame, and colored fills. Because Factorio applies damage before scripts run, a single hit that exceeds the turret's native HP may still be lethal before Shield can respond.
