...
- Ammo Productivity stores the last loaded ammo item and quality on the Veteran Core. Each rank adds 1% ammo spending productivity: spent ammo fills a purple horizontal progress bar under the ammo slot in the custom stats panel, and every full bar restores one matching ammo item after a shot. Because Factorio does not expose a direct turret-fired event, runtime logic observes loaded-ammo deltas when the turret deals damage.
...
- Ammo Productivity adds 1% ammo spending productivity per core rank. Spent ammo fills the custom ammo productivity bar, and a full bar restores one matching ammo item after a shot.
...
- The Ammo Productivity bar uses a purple custom style on Factorio's native horizontal progressbar widget, placed under the loaded-ammo slot in the stats panel.
