...
- Empty-turret Veteran Core selection uses the whole anchored panel width. The picker has base/specialization checkboxes above a Factorio-style sortable table; table headers cycle ascending, descending, and no explicit sort while showing a vanilla utility-arrow sprite for the active direction. Core rows use visible delimiters, a colored specialization line, right-aligned preview stats, and an exact-slot `+` install action.
