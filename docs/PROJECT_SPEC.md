...
- Ammo Productivity stores the last loaded ammo item, quality, and current magazine ammo on the Veteran Core. Each rank adds 1% raw magazine productivity, then runtime converts raw productivity into effective refill progress with `raw / (raw + 1)` so investment can scale indefinitely without reaching free ammo. Spent ammo fills a purple horizontal progress bar in its own stat row after Magazine, and every full bar restores +1 ammo inside the current loaded magazine when there is room. The refill is capped by the ammo prototype's normal magazine size, does not overfill `11/10`, and does not create full ammo items. Because Factorio does not expose a direct turret-fired event, runtime logic observes loaded-magazine ammo deltas when the turret deals damage.
...
- The Evolution column does not show feeder status; unlocked element panels always show the current rank, technical effect, next material requirement, and passive progress bar. Element choices use compact card-style rows with icon/title, full-width descriptions, separator, and technical value rows. Specialization and sub-specialization choices use two-column cards: icon/title/flavor text on the left and vertically stacked mechanical effects plus the `Pick` action on the right. Unlocked element panels must not expand under the scrollbar.
- Stats rows show compact final values in the panel. Additive/multiplier formulas live in the stat-name info hover, while quality-specific HP and range variants live on the quality diamond beside the value.
...
- Ammo and Magazine are separate stat rows: Ammo shows the loaded ammo stack, Magazine shows current rounds in the active magazine, and the Ammo Productivity bar uses a purple custom style on Factorio's native horizontal progressbar widget in a separate stat row immediately after Magazine.
