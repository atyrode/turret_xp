...
- Ammo Productivity stores the last loaded ammo item, quality, and current magazine ammo on the Veteran Core. Each rank adds 1% magazine productivity: spent ammo fills a purple horizontal progress bar in its own stat row after Magazine, and every full bar restores +1 ammo inside the current loaded magazine when there is room. The refill is capped by the ammo prototype's normal magazine size, does not overfill `11/10`, and does not create full ammo items. Because Factorio does not expose a direct turret-fired event, runtime logic observes loaded-magazine ammo deltas when the turret deals damage.
...
- Ammo Productivity adds 1% magazine productivity per core rank. Spent ammo fills the custom ammo productivity bar, and a full bar restores +1 ammo inside the current loaded magazine up to that magazine's normal capacity.
...
- The Ammo Productivity bar uses a purple custom style on Factorio's native horizontal progressbar widget, placed in a separate stat row immediately after Magazine.
