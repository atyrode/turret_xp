...
V0.11 keeps the vanilla turret GUI as the main interaction while rebuilding the attached Turret XP panel around a Factorio-native anchored GUI shell. Installed cores use a two-column dashboard: Veteran Core identity, naming, XP, dev controls, and bounded Stats on the left; Evolution/progression choices in a fixed-header bounded pane on the right. Empty turrets use the full Turret XP body as an exact Veteran Core picker with persisted sort/filter preferences, neutral preview stats, exact-slot install actions, and an optional logistic request for one delivered Veteran Core. Turret identity is an explicit player choice through a movable Veteran Core item, while ordinary gun turrets stay stackable and inventory-friendly. Installed cores can optionally bind to their turret body for quick moves: mining a bound veteran turret returns one tagged placeable turret item instead of a separate turret plus core. Bound items place a hidden bound-only placeholder that is immediately converted into a real gun turret with the stored core profile, which keeps normal gun-turret replacement ghosts tied to the normal gun turret item. Installing a core creates an invisible inserter-fed input on the turret tile whenever selected elements need next-rank materials, avoiding a visible fake chest beside the turret while routing ammo back into the turret. Empty-core logistic requests use a separate hidden requester helper and bounded refresh loop so bots can deliver a Veteran Core without turning the turret into a general storage entity. Nearby inserters that are actually sourcing needed materials are pointed at the hidden input and can receive multiple active element filters at once. When input is not needed, inserters are restored to the turret target so ammo logistics keep behaving normally. Specialization and sub-specialization choices use hidden gun-turret variants with prototype stat changes generated in `data-final-fixes.lua`, so they inherit final base gun-turret edits from mods that run during `data-updates.lua`. Shield and Resistance are intentionally scripted instead of variant-backed: Shield absorbs non-lethal incoming damage before HP and recharges in small increments after a delay with no incoming damage, while Resistance refunds part of remaining non-lethal final incoming damage after Factorio's vanilla resistance calculation. Body swaps are queued until the turret GUI closes so Factorio does not reset the whole vanilla window position. Combat XP is weighted by surface, target type, platform travel state, and Veteran Training at award time so platform asteroid defense is tunable without multiplying old XP totals or passively overleveling cores. The implementation is split into runtime modules under `scripts/control/` and data-stage modules under `prototypes/`.
...
- New core profiles start at level 0 with zero XP, kills, kill credit, damage, total XP, custom name, hidden floating-label components, manual automation, and empty evolution choices.
...
- XP is derived from XP-weighted lifetime damage, XP-weighted kill credit, runtime-global XP settings, award-time Veteran Training ranks, and optional dev XP.
...
- Non-asteroid combat by turrets on a space-platform surface applies a `0.1x` multiplier before damage or kill credit reaches the XP counters.
- Combat against asteroids or asteroid chunks uses configurable target multipliers instead of stacking with the platform surface multiplier: `0.2x` while the platform is travelling between space locations and `0.05x` while the platform is stopped, paused, or otherwise not advancing.
- Veteran Training applies to newly earned combat XP before it reaches `xp_damage` or `xp_kill_credit`. Removing Veteran Training ranks must not lower already-earned XP or levels; old profiles are migrated once to preserve the XP they had under the previous total-XP multiplier behavior.
...
- Automation presets can be selected from the installed-core panel. `Apply` spends currently available points once using the selected preset. `Auto` keeps the preset active for future point gains. Presets spend deterministic core and augment priorities, choose compatible specialization/sub-specialization/elements when unlocked, and report conflicts without overwriting existing conflicting manual identity choices.
...
- The profile can be named. Name, level, and unspent point summary can each be independently included in the world label above the current turret body.
- Label color can be chosen through presets or RGB sliders stored on the Veteran Core profile. Color controls are hidden until at least one floating-label component is enabled. Preset cycling records a preset name, while RGB slider edits mark the color as custom. Floating labels render with `rendering.draw_text` so custom RGB colors apply directly without generated label-color prototypes.
- Empty turrets can request one delivered Veteran Core. The hidden requester helper is created only while the empty turret request is active, asks for `turret-xp-veteran-core`, installs the first delivered tagged core profile, and is destroyed when the request is cancelled, fulfilled, or the turret is removed. Delivered contents spill during teardown rather than being deleted.
- Blueprint tags and entity-settings copy/paste carry label visibility, label color, automation preset/Auto state, and empty-turret core-request policy. They do not carry source XP, level, kills, damage, or custom name.
...
- Veteran Training increases newly earned combat XP from damage and kill credit.
...
- The panel includes a Veteran Core slot-style control for install/extract, naming, optional quick-move binding, floating-label component toggles, label color swatch/picker trigger, automation preset controls, and empty-turret core requests. The installed-core header shows the core name, level, and bound/unbound state beside the slot, while extract and Bind/Unbind sit in the right-side action toolbar so the quick-move state reads as part of that core-slot interaction. Installed-core naming and floating-label controls render as a compact shallow form: the name row keeps the custom name field, the display row exposes `Name`, `Level`, and `Unspent` toggles independently, and color controls appear only while at least one floating-label component is enabled.
- When the opened turret is on a space platform, the Veteran Core panel also lists tagged cores found in that platform's hub inventory. Each separated row represents the exact hub inventory slot, with level, specialization, neutral preview stats, and an install button. Installed platform cores can be sent back to the same hub if it has room.
- The Evolution column does not show feeder status; unlocked element panels always show the current rank, technical effect, next material requirement, and passive progress bar. Element, specialization, and sub-specialization choices use compact card rows with icon/title first, the `Pick` action in the title-row action slot, full-width flavor text, then full-width technical rows or stat tables. Unlocked element panels must not expand under the scrollbar.
...
- The XP bar shows an adjacent final combat XP rate only when the opened turret has a non-base XP modifier. Ground combat with no Veteran Training shows no extra rate line; platform asteroid combat shows the travelling or stopped asteroid final rate, compounded with Veteran Training when present, and its info hover lists the formula plus the non-asteroid platform rate.
...
- Pre-publish validation: pull requests and `main` run GitHub Actions package validation for package-payload changes, including a stale generated-public-assets check. Headless tests run in CI when Mod Portal download credentials are configured and runtime/test paths change, and passing headless runs print benchmark timing plus process resource metrics where the platform exposes them. When a merge to `main` leaves `info.json` at a version whose matching GitHub Release/tag does not exist, Auto Release creates `v<info.json version>` and dispatches the Release workflow when the package asset is missing. The Release workflow validates the tag, verifies generated public assets, runs the headless suite, builds and attaches the package to the GitHub Release, then publishes that exact release package to the Mod Portal. GitHub Release notes, Mod Portal copy, and the public GitHub Pages source are generated from `info.json`, `changelog.txt`, and `docs/public-copy.json`. Local checkouts may create or update the GitHub Release through `scripts/release.sh` only as a fallback; local checkouts must not publish Mod Portal releases. The packaged zip includes root `README.md`, `changelog.txt`, locale, Lua/prototype source, and root `thumbnail.png` when present; internal `docs/` files and generated public-site assets remain outside the mod package.
