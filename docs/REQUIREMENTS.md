...
- Show installed-core Build mode controls with Enter/Exit, finite planned ranks, loop priorities, Follow build in live mode, and clear visual tinting while previewing a build. The normal live Build control must remain a single unframed row, detailed target summaries must render only inside Build mode, and loop-priority checkboxes must render as labeled rows under their upgrade or augment so the normal live turret panel remains compact.
