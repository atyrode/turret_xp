...
- The headless test mod intentionally patches vanilla gun turret range during `data-updates.lua`, then asserts Turret XP's `data-final-fixes.lua` variants inherit that range. This covers the K2 Spaced Out-style bug where Range rank 1 could otherwise reduce a modded gun turret from 25 range to an early vanilla-derived 19 range.
