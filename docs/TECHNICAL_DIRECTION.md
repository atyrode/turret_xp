...
- `LuaRendering::draw_rectangle` is used for the scripted HP/Shield status stack because local library-mod research found overlay references but no reusable world-space health/shield bar dependency. The stack is attached to the turret entity through render targets and does not add data-stage prototypes.
