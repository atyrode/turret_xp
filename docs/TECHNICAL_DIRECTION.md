...
- `LuaRendering::draw_rectangle` is used for the scripted segmented Shield bar because local library-mod research found overlay references but no reusable world-space health/shield bar dependency. The bar is attached to the turret entity through render targets and does not add data-stage prototypes.
