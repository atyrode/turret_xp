...
- The same `LuaSurface::platform` check is used for the 0.6.2 space-combat XP multiplier. Raw `damage` and `kill_credit` stay display/credit totals, while `xp_damage` and `xp_kill_credit` are the counters used by progression math.
...
- V0.6.2 specialization and Range augment stats are prototype-backed. Feeder placement is an invisible colocated input with ammo forwarding and cap-closing fuel behavior; edge cases around full turret ammo inventories, unsupported ammo, furnace-like element fuel readability, unusual inserter layouts, platform hub inventory fullness, platform mining buffers, or space-combat XP pacing still need playtesting.
